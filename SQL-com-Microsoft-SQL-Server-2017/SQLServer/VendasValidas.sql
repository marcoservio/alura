...
SELECT NF.CPF, SUBSTRING(CONVERT(VARCHAR, NF.[DATA], 120), 1, 7) AS ANO_MES, 
SUM(INF.QUANTIDADE) QUANTIDADE_MES FROM [NOTAS FISCAIS] NF
INNER JOIN [ITENS NOTAS FISCAIS] INF
ON NF.NUMERO = INF.NUMERO
GROUP BY NF.CPF, SUBSTRING(CONVERT(VARCHAR, NF.[DATA], 120), 1, 7)

SELECT C.NOME, C.[VOLUME DE COMPRA] FROM [TABELA DE CLIENTES] C

SELECT C.NOME, CQ.ANO_MES, CQ.QUANTIDADE_MES, C.[VOLUME DE COMPRA]
FROM
(SELECT NF.CPF, SUBSTRING(CONVERT(VARCHAR, NF.[DATA], 120), 1, 7) AS ANO_MES, 
SUM(INF.QUANTIDADE) QUANTIDADE_MES FROM [NOTAS FISCAIS] NF
INNER JOIN [ITENS NOTAS FISCAIS] INF
