...
def analisar_transacao(lista_de_transacoes):
    ...
    resposta = cliente.chat.completions.create(
        messages=lista_mensagens, model=modelo, temperature=0
    )
...
def gera_parecer(transacao):
    ...
    """
    lista_mensagens = [{"role": "system", "content": prompt_sistema}]
...
transacoes_analizadas = analisar_transacao(lista_de_transacoes)

for transacao in transacoes_analizadas["transacoes"]:
    if transacao["status"] == "Possível Fraude":
        ...
        salva(
            f"transacao-{id_transacao}-{produto_transacao}-{status_transacao}.txt",
            recomendacao,
        )
